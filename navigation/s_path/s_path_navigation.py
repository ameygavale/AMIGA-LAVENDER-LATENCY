...
def create_turn_segment(
    previous_pose: Pose3F64, next_frame_b: str, angle: float, spacing: float = 0.05
) -> list[Pose3F64]:
    """Create a turn segment with waypoints."""
    segment_poses: list[Pose3F64] = []
    num_waypoints = int(abs(angle) / spacing)
    
    for i in range(num_waypoints + 1):
        segment_angle = i * spacing if angle > 0 else -i * spacing
        waypoint = Pose3F64(
    ...
    return segment_poses[1:]  # Skip first pose
